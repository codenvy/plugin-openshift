...
/**
 * Utility class, that contains validation methods for various OpenShift entities
 *
...
public class OpenshiftValidator {
  ...
  private static final String LABEL_NAME_PREFIX =
   ...
   *
   * @param name OpenShift project name
  ...
  /**
   * @param name OpenShift application name
  ...
  /**
   * @param variable OpenShift application variable name
  ...
  /**
   * @param labelName OpenShift application label name
  ...
  public static boolean isLabelNameValid(String labelName) {
    ...
    if (splitted.length == 2) {
      ...
      return !(Strings.isNullOrEmpty(prefix)
          || prefix.length() > 253
          || !prefix.matches(LABEL_NAME_PREFIX)
          || Strings.isNullOrEmpty(name)
          || name.length() > 63
          || !name.matches(LABEL_NAME));
  ...
  /**
   * @param labelValue OpenShift application label name
   * @return true if label name is valid, false otherwise
   */
