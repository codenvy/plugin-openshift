...
public class Application {
  /**
   * {@link BuildConfig} is a basic object of application. We can search other objects by this
   * object.<br>
   * {@link BuildConfig} is linked to {@link ImageStreamTag} by {@link ObjectReference} that you can
   * get by calling <br>
   * {@link BuildConfig#getSpec()} and then {@link BuildConfigSpec#getOutput()} and then {@link
   * BuildOutput#getTo()}.
   *
   * <p>{@link ObjectReference} should return {@code "ImageStreamTag"} when you call {@link
   * ObjectReference#getKind()} <br>
   * and image stream tag name as {@code "imageStreamName:tagName"} when you call {@link
   * ObjectReference#getName()}
   */
   ...
   *
   * <p>If {@link #outputImageStream} doesn't have required build tag, then this reference equals to
   * <br>
   * {@code "imageStreamName:tagName"}.<br>
   * And if {@link #outputImageStream} has required tag - the value can be retrieved from {@link
   * #outputImageStream}<br>
   * You should find needed instance of {@link NamedTagEventList} by calling {@link
   * ImageStream#getStatus()}<br>
   * and then {@link ImageStreamStatus#getTags()}. Then you can get last item from {@link
   * NamedTagEventList#getItems()} and then <br>
   * you can get tag reference by calling {@link TagEvent#getDockerImageReference()} from found
   * instance
   */
  private String imageStreamTagReference;

  /**
   * {@link DeploymentConfig} is linked with {@link BuildConfig} by {@link
   * #imageStreamTagReference}.
   *
   * <p>{@link Container#getImage()} of {@link DeploymentConfig} should equal to {@link
   * #imageStreamTagReference}.<br>
   * {@link Container#getImage()} can have no imageStreamTag. By default it equals to 'latest'.<br>
   * Needed instance of {@link Container} can be retrieved by next calling {@link
   * DeploymentConfig#getSpec()} <br>
   ...
   *
   * <p>{@link DeploymentConfig} creates new pod that will contain all labels from {@link
   * ObjectMeta} <br>
   * that you can get by calling next methods {@link DeploymentConfig#getSpec()} and then {@link
   * DeploymentConfigSpec#getTemplate()}<br>
   * and then {@link PodTemplateSpec#getMetadata()} {@link ObjectMeta}. And {@link Service} selects
   * pods by selectors that you can get by {@link Service#getSpec()} and then <br>
   * {@link ServiceSpec#getSelector()}. So {@link Service} relates to {@link DeploymentConfig} if
   * any of pair(key=value) from <br>
   * service's {@code selector} equals to any of pair(key=value) from PodTemplateSpec's labels
   */
  private List<Service> services;

  /**
   * {@link Route} is linked to {@link Service} by {@link ObjectReference} that you can get by
   * calling <br>
   * {@link Route#getSpec()} and then {@link RouteSpec#getTo()}.
   *
   * <p>{@link ObjectReference} should return {@code "Service"} when you call {@link
   * ObjectReference#getKind()}}<br>
