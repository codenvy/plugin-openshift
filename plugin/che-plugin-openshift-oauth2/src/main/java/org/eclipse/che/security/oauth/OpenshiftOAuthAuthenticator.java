...
public class OpenshiftOAuthAuthenticator extends OAuthAuthenticator {
  ...
  public OpenshiftOAuthAuthenticator(
      ...
      @Nullable @Named("che.openshift.api.endpoint") String openshiftApiEndpoint)
      throws IOException {
  ...
  @Override
  public org.eclipse.che.security.oauth.shared.User getUser(OAuthToken accessToken)
      throws OAuthAuthenticationException {
    final User currentUser =
        requestUser(openshiftApiEndpoint + "/oapi/v1/users/~", accessToken.getToken());

    return new OpenshiftUser()
        .withEmail(currentUser.getMetadata().getName())
  ...
  // Need for working with https://api.codenvy.openshift.com/ server that has broken sll
  // certificates.
  static {
