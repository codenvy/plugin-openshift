...
public class ConfigureServiceViewImpl extends Window implements ConfigureServiceView {
  ...
  interface ConfigureServiceViewUiBinder
      extends UiBinder<DockLayoutPanel, ConfigureServiceViewImpl> {}
  ...
  @UiField SimplePanel envVariablesPanel;

  @UiField SimplePanel labelsPanel;

  @UiField Button addLabelButton;

  @UiField Label labelsErrorLabel;

  @UiField Label environmentsErrorLabel;
  ...
  @Inject
  public ConfigureServiceViewImpl(
  ...
  private CellTable<Parameter> createVariablesTable(final CellTableResources tableResources) {
    ...
    envVariablesTable.setKeyboardSelectionPolicy(
        HasKeyboardSelectionPolicy.KeyboardSelectionPolicy.DISABLED);
    envVariablesProvider.addDataDisplay(envVariablesTable);

    Column<Parameter, String> nameColumn =
        new Column<Parameter, String>(new TextCell()) {
          @Override
    ...
    TextInputCellWithPlaceHolder variablesCell =
        new TextInputCellWithPlaceHolder(locale.environmentVarialbesValuePlaceholder());
    Column<Parameter, String> valueColumn =
        new Column<Parameter, String>(variablesCell) {
          @Override
          public void onBrowserEvent(
              Cell.Context context, Element elem, Parameter parameter, NativeEvent event) {
            super.onBrowserEvent(context, elem, parameter, event);

            if (event.getType().equals(BrowserEvents.KEYUP)) {
              TextInputCellWithPlaceHolder cell =
                  (TextInputCellWithPlaceHolder)
                      envVariablesTable.getColumn(context.getColumn()).getCell();
    ...
    valueColumn.setFieldUpdater(
        new FieldUpdater<Parameter, String>() {
          @Override
  ...
  private CellTable<KeyValue> createLabelsTable(
      CellTableResources tableResources, final Predicate<String> labelValidator) {
    labelsTable = new CellTable<>(50, tableResources);
    labelsTable.setTableLayoutFixed(true);
    labelsTable.setKeyboardSelectionPolicy(
        HasKeyboardSelectionPolicy.KeyboardSelectionPolicy.DISABLED);
    labelsProvider.addDataDisplay(labelsTable);

    final ValidatedInputCell keyTextInputCell =
        new ValidatedInputCell(labelValidator, locale.labelNamePlaceholder());
    final Column<KeyValue, String> keyColumn =
        new Column<KeyValue, String>(keyTextInputCell) {
          @Override
    ...
    keyColumn.setFieldUpdater(
        new FieldUpdater<KeyValue, String>() {
          @Override
    ...
    ValidatedInputCell valueTextInputCell =
        new ValidatedInputCell(labelValidator, locale.labelValuePlaceholder());
    Column<KeyValue, String> valueColumn =
        new Column<KeyValue, String>(valueTextInputCell) {
          @Override
          public String getCellStyleNames(Cell.Context context, KeyValue keyValue) {
    ...
    Column<KeyValue, String> removeColumn =
        new Column<KeyValue, String>(new ButtonCell()) {
          @Override
  ...
  public void showLabelsError(String labelMessage, String tooltipMessage) {
    ...
    if (!Strings.isNullOrEmpty(tooltipMessage)) {
      labelsErrorTooltip =
          Tooltip.create(
              (elemental.dom.Element) labelsErrorLabel.getElement(),
