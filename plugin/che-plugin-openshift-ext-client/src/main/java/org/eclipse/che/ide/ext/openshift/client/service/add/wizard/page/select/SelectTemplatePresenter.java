 ...
 */
@Singleton
public class SelectTemplatePresenter extends AbstractWizardPage<NewServiceRequest>
    implements SelectTemplateView.ActionDelegate {
  ...
  @Inject
  public SelectTemplatePresenter(
  ...
  public void init(NewServiceRequest dataObject) {
    ...
    final String namespace =
        getAttributeValue(appContext.getRootProject(), OPENSHIFT_NAMESPACE_VARIABLE_NAME);

    view.showLoadingTemplates();

    List<Template> allTemplates = new ArrayList<>();
    getTemplates(DEFAULT_NAMESPACE, allTemplates)
        .then(getTemplates(namespace, allTemplates))
        .then(filterByCategory(DATABASE_TAG))
        .then(addTemplates())
        .catchError(handleError());
  }

  private Promise<List<Template>> getTemplates(
      String nameSpace, final List<Template> allTemplates) {
    return client
        .getTemplates(nameSpace)
        .then(
            new Function<List<Template>, List<Template>>() {
              @Override
  ...
  private Function<List<Template>, List<Template>> filterByCategory(
    ...
    return new Function<List<Template>, List<Template>>() {
      ...
      public List<Template> apply(List<Template> templates) throws FunctionException {
        ...
        for (final Template template : templates) {
          ...
          // For adding service we need to use only templates with one deployment config and
          // database tag:
    ...
    return new Operation<PromiseError>() {
      ...
      public void apply(PromiseError promiseError) throws OperationException {
        notificationManager.notify(
            locale.getListTemplatesFailed() + " " + promiseError.getMessage(), FAIL, EMERGE_MODE);
