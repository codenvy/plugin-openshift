...
public class BuildConfigService {
  ...
  public BuildConfig createBuildConfig(
      @PathParam("namespace") String namespace, BuildConfig buildConfig)
    ...
    if (!ResourceKind.BUILD_CONFIG.equals(buildConfig.getKind())) {
      throw new BadRequestException(
          buildConfig.getKind() + " cannot be handled as a " + ResourceKind.BUILD_CONFIG);
    }
  ...
  public List<BuildConfig> getBuildConfigs(
    ...
    List<IBuildConfig> buildConfigs =
        clientFactory.getOpenshiftClient().list(ResourceKind.BUILD_CONFIG, namespace, labels);
    return buildConfigs
        .stream()
  ...
  public BuildConfig getBuildConfig(
      @PathParam("namespace") String namespace, @PathParam("buildConfig") String buildConfig)
      throws UnauthorizedException, ServerException {

    return toDto(
        BuildConfig.class,
        clientFactory.getOpenshiftClient().get(ResourceKind.BUILD_CONFIG, buildConfig, namespace));
  }
  ...
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public BuildConfig updateBuildConfig(
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public List<WebHook> getBuildConfigWebHooks(
      @PathParam("namespace") String namespace, @PathParam("buildConfig") String buildConfigName)
      throws UnauthorizedException, ServerException {
    final IClient client = clientFactory.getOpenshiftClient();
    final IBuildConfig buildConfig =
        client.get(ResourceKind.BUILD_CONFIG, buildConfigName, namespace);
    ...
    for (IBuildTrigger iBuildTrigger : buildConfig.getBuildTriggers()) {
      if (iBuildTrigger instanceof IWebhookTrigger) {
        ...
        result.add(
            newDto(WebHook.class)
                .withUrl(webhookTrigger.getWebhookURL())
