...
public class OpenshiftServiceClientImpl implements OpenshiftServiceClient {
  ...
  @Inject
  public OpenshiftServiceClientImpl(
  ...
  private String openshiftPath() {
    return appContext.getDevMachine().getWsAgentBaseUrl() + "/openshift";
  }

  @Override
  public Promise<OpenshiftServerInfo> getServerInfo() {
    return asyncRequestFactory
        .createGetRequest(openshiftPath())
  ...
  public Promise<List<Template>> getTemplates(final String namespace) {
    return asyncRequestFactory
        .createGetRequest(openshiftPath() + "/namespace/" + namespace + "/template")
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<Template> processTemplate(final String namespace, final Template template) {
    return asyncRequestFactory
        .createPostRequest(
            openshiftPath() + "/namespace/" + namespace + "/template/process", template)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<List<Project>> getProjects() {
    return asyncRequestFactory
        .createGetRequest(openshiftPath() + "/project")
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<Project> createProject(final ProjectRequest request) {
    return asyncRequestFactory
        .createPostRequest(openshiftPath() + "/project", request)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<BuildConfig> createBuildConfig(final BuildConfig config) {
    String url =
        openshiftPath() + "/namespace/" + config.getMetadata().getNamespace() + "/buildconfig";
    return asyncRequestFactory
        .createPostRequest(url, config)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<BuildConfig> updateBuildConfig(final BuildConfig config) {
    ...
    return asyncRequestFactory
        .createRequest(PUT, url, config, false)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  @Override
  public Promise<List<BuildConfig>> getBuildConfigs(
      final String namespace, final String application) {
  ...
  public Promise<ImageStream> createImageStream(final ImageStream stream) {
    String url =
        openshiftPath() + "/namespace/" + stream.getMetadata().getNamespace() + "/imagestream";
    return asyncRequestFactory
        .createPostRequest(url, stream)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  @Override
  public Promise<List<ImageStream>> getImageStreams(
      final String namespace, final String application) {
  ...
  public Promise<ImageStream> updateImageStream(final ImageStream imageStream) {
    final String url =
        openshiftPath()
            + "/namespace/"
            + imageStream.getMetadata().getNamespace()
            + "/imagestream/"
            + imageStream.getMetadata().getName();
    return asyncRequestFactory
        .createRequest(PUT, url, imageStream, false)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  @Override
  public Promise<ImageStreamTag> getImageStreamTag(
      final String namespace, final String imageStream, final String tag) {
    final String url =
        openshiftPath() + "/namespace/" + namespace + "/imagestream/" + imageStream + "/tag/" + tag;
    return asyncRequestFactory
        .createGetRequest(url)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<DeploymentConfig> createDeploymentConfig(final DeploymentConfig config) {
    String url =
        openshiftPath() + "/namespace/" + config.getMetadata().getNamespace() + "/deploymentconfig";
    return asyncRequestFactory
        .createPostRequest(url, config)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  @Override
  public Promise<List<DeploymentConfig>> getDeploymentConfigs(
      final String namespace, final String application) {
  ...
  public Promise<Route> createRoute(final Route route) {
    return asyncRequestFactory
        .createPostRequest(
            openshiftPath() + "/namespace/" + route.getMetadata().getNamespace() + "/route", route)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<Route> updateRoute(final Route route) {
    ...
    return asyncRequestFactory
        .createRequest(PUT, url, route, false)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  public Promise<Service> createService(final Service service) {
    String url =
        openshiftPath() + "/namespace/" + service.getMetadata().getNamespace() + "/service";
    return asyncRequestFactory
        .createPostRequest(url, service)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
  ...
  @Override
  public Promise<List<ReplicationController>> getReplicationControllers(
      final String namespace, final String application) {
  ...
  @Override
  public Promise<ReplicationController> updateReplicationController(
      final ReplicationController controller) {
    final String url =
        openshiftPath()
            + "/namespace/"
            + controller.getMetadata().getNamespace()
            + "/replicationcontroller/"
            + controller.getMetadata().getName();
    return asyncRequestFactory
        .createRequest(PUT, url, controller, false)
        .header(CONTENT_TYPE, MimeType.APPLICATION_JSON)
        .header(ACCEPT, MimeType.APPLICATION_JSON)
