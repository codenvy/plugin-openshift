...
public class ClientFactory {
  ...
  public ClientFactory(
      @Named("che.openshift.api.endpoint") String openshiftApiEndpoint,
      RemoteOAuthTokenProvider provider) {
    this.openshiftApiEndpoint = openshiftApiEndpoint;
    this.provider = provider;
    this.token2clientCache =
        CacheBuilder.newBuilder()
            .maximumSize(1000)
            .expireAfterAccess(10, TimeUnit.MINUTES)
            .build(
                new CacheLoader<String, IClient>() {
                  @Override
    ...
    this.infoClient =
        new com.openshift.restclient.ClientFactory()
            .create(openshiftApiEndpoint, new NoopSSLCertificateCallback());
  }

  public OpenshiftServerInfo getClientInfo() {
    final String openShiftAPIVersion = infoClient.getOpenShiftAPIVersion();
    final String openshiftEndpoint =
        (OpenShiftAPIVersion.v1beta3.toString().equals(openShiftAPIVersion) ? "osapi" : "oapi")
            + "/"
            + openShiftAPIVersion;
    return newDto(OpenshiftServerInfo.class)
        .withOpenshiftEndpoint(normalize(infoClient.getBaseURL().toString()) + openshiftEndpoint);
  }
   ...
   *
   * @throws UnauthorizedException when user did not have access token to openshift server
   * @throws ServerException when some exception occurs during getting of access token
   */
