...
public class BuildService {
  ...
  public Build createBuild(
    ...
    final IBuildTriggerable buildConfigTrigger =
        iBuildConfig.getCapability(IBuildTriggerable.class);
  ...
  public List<Build> getBuilds(
    ...
    List<IBuild> imageStreams =
        clientFactory.getOpenshiftClient().list(ResourceKind.BUILD, namespace, labels);
    return imageStreams
        .stream()
  ...
  @Path("/{build}")
  @Produces(MediaType.APPLICATION_JSON)
  public Build getBuild(@PathParam("namespace") String namespace, @PathParam("build") String build)
      throws UnauthorizedException, ServerException {

    return toDto(
        Build.class, clientFactory.getOpenshiftClient().get(ResourceKind.BUILD, build, namespace));
  }
}
