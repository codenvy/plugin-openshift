...
public class ProjectService {
  ...
  public Project createProject(ProjectRequest project)
    ...
    if (!ResourceKind.PROJECT_REQUEST.equals(project.getKind())) {
      throw new BadRequestException(
          project.getKind() + " cannot be handled as a " + ResourceKind.PROJECT_REQUEST);
    }
  ...
  @Path("/{project}")
  @Produces(MediaType.APPLICATION_JSON)
  public Project getProject(@PathParam("project") String project)
      throws UnauthorizedException, ServerException {

    return toDto(
        Project.class,
        clientFactory.getOpenshiftClient().get(ResourceKind.PROJECT, project, project));
  }
  ...
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public Project updateProject(@PathParam("project") String projectName, Project project)
  ...
  @DELETE
  @Path("/{project}")
  public void removeProject(@PathParam("project") String projectName)
      throws UnauthorizedException, ServerException {
    final IClient client = clientFactory.getOpenshiftClient();
    client.delete(
        toOpenshiftResource(
            client,
            newDto(Project.class)
                .withKind(ResourceKind.PROJECT)
                .withMetadata(newDto(ObjectMeta.class).withName(projectName))));
  }
}
