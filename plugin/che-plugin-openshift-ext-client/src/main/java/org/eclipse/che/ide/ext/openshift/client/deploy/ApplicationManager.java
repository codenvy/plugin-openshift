...
public class ApplicationManager {
  ...
  public ApplicationManager(
      OpenshiftServiceClient openShiftClient, OpenshiftLocalizationConstant locale) {
    this.openShiftClient = openShiftClient;
    this.locale = locale;
  }

  /** @return a list of pairs, where first is namespace, second - application name */
  public Promise<List<Pair<String, String>>> getApplicationNamesByNamespaces() {
    final Executor.ExecutorBody<List<Pair<String, String>>> body =
        new Executor.ExecutorBody<List<Pair<String, String>>>() {
          @Override
          public void apply(
              final ResolveFunction<List<Pair<String, String>>> resolve,
              final RejectFunction reject) {
            openShiftClient
                .getProjects()
                .then(
                              ...
                              openShiftClient
                                  ...
                                  .then(
                                      new Operation<List<BuildConfig>>() {
                                        @Override
                                        public void apply(List<BuildConfig> buildConfigs)
  ...
  /**
   * @param buildConfig build config for searching other application objects
  ...
  public Promise<Application> findApplication(final BuildConfig buildConfig) {
    final Executor.ExecutorBody<Application> body =
        new Executor.ExecutorBody<Application>() {
          @Override
          public void apply(
              final ResolveFunction<Application> resolve, final RejectFunction reject) {
            ...
            if (split.length != 2) {
              reject.apply(
                  JsPromiseError.create(locale.buildConfigHasInvalidTagName(imageTagName)));
              return;
            }
            ...
            openShiftClient
                .getImageStream(namespace, imageStreamName)
                .thenPromise(
                    new Function<ImageStream, Promise<String>>() {
                      @Override
                      public Promise<String> apply(ImageStream imageStream)
                          throws FunctionException {
                        application.setOutputImageStream(imageStream);
                        return getDockerImageReference(
                            imageStreamName, imageStreamTag, imageStream);
                      }
                    })
                .thenPromise(
                    new Function<String, Promise<List<DeploymentConfig>>>() {
                      @Override
                      public Promise<List<DeploymentConfig>> apply(String imageStreamTagReference)
                          throws FunctionException {
                ...
                .thenPromise(
                    new Function<List<DeploymentConfig>, Promise<List<Service>>>() {
                      @Override
                      public Promise<List<Service>> apply(List<DeploymentConfig> deploymentConfigs)
                          throws FunctionException {
                        application.setDeploymentConfigs(
                            filterDeploymentConfigsByImageTag(
                                deploymentConfigs, application.getImageStreamTagReference()));
                        return openShiftClient.getServices(namespace, null);
                      }
                    })
                .thenPromise(
                    new Function<List<Service>, Promise<List<Route>>>() {
                      @Override
                      public Promise<List<Route>> apply(List<Service> services)
                          throws FunctionException {
                        application.setServices(
                            filterServicesByDeploymentConfigs(
                                services, application.getDeploymentConfigs()));
                        return openShiftClient.getRoutes(namespace, null);
                      }
                    })
                .then(
                    new Operation<List<Route>>() {
                      @Override
                      public void apply(List<Route> routes) {
                        application.setRoutes(
                            filterRoutesByServices(routes, application.getServices()));
  ...
  /**
   * Returns new list that contains only those {@link Route} instances from given list of routes
   * <br>
  ...
  /**
   * Returns new list that contains only those {@link Service} instances from given list of services
   * <br>
   ...
   */
  private List<Service> filterServicesByDeploymentConfigs(
      List<Service> services, List<DeploymentConfig> deploymentConfigs) {
    ...
    for (DeploymentConfig deploymentConfig : deploymentConfigs) {
      for (Map.Entry<String, String> stringStringEntry :
          deploymentConfig.getSpec().getTemplate().getMetadata().getLabels().entrySet()) {
        labels.add(Pair.of(stringStringEntry.getKey(), stringStringEntry.getValue()));
      }
    }
    for (Service service : services) {
      for (Map.Entry<String, String> stringStringEntry :
          service.getSpec().getSelector().entrySet()) {
  ...
  /**
   * Returns new list that contains only those {@link DeploymentConfig} instances from given list of
   * deployment configs <br>
   ...
   */
  private List<DeploymentConfig> filterDeploymentConfigsByImageTag(
      List<DeploymentConfig> deploymentConfigs, String imageStreamTagReference) {
    List<DeploymentConfig> filtered = new ArrayList<>();
    for (DeploymentConfig deploymentConfig : deploymentConfigs) {
      for (Container container :
          deploymentConfig.getSpec().getTemplate().getSpec().getContainers()) {
  ...
  /** Returns ImageStreamTagReference. See {@link Application#imageStreamTagReference} */
  private Promise<String> getDockerImageReference(
      String imageStreamName, String imageStreamTag, ImageStream imageStream) {
    if (imageStream.getStatus().getTags().isEmpty()) {
          ...
          if (namedTagEventList.getItems().isEmpty()) {
            return Promises.reject(
                JsPromiseError.create(
                    locale.imageSteamHasInvalidTagError(imageStreamName, imageStreamTag)));
          } else {
            return Promises.resolve(namedTagEventList.getItems().get(0).getDockerImageReference());
          }
        }
      }
      ...
      return Promises.reject(
          JsPromiseError.create(
              locale.imageStreamDoesNotHaveAnyTag(imageStreamName, imageStreamTag)));
    }
  }

  /**
   * Updates objects from given {@link Application} if there are changes after adding label
   * 'application' <br>
   * with given value to all objects from {@link Application}.
   *
   * @param application instance of {@link Application} for updating of its configs
   * @param applicationName value of label application that will be adding to all objects from
   *     {@link Application}
   */
  public Promise<Application> updateOpenshiftApplication(
      final Application application, final String applicationName) {
    return updateOpenshiftApplication(
        application, applicationName, application.getBuildConfig().getSpec().getSource());
  }
  /**
   * Updates objects from given {@link Application} if there are changes after setting new given
   * {@link BuildSource} <br>
   * for {@link Application#buildConfig} and adding label 'application' with given value to all
   * objects from {@link Application}.
   *
   * @param application instance of {@link Application} for updating of its configs
   * @param applicationName value of label application that will be adding to all objects from
   *     {@link Application}
   * @param newBuildSources instance of {@link BuildSource} that will be set to {@link BuildConfig}
   *     from {@link Application#getBuildConfig()}
   */
  public Promise<Application> updateOpenshiftApplication(
        ...
        new Executor.ExecutorBody<Application>() {
          ...
          public void apply(
            ...
            for (Service service : application.getServices()) {
              oldApplicationName =
                  service.getMetadata().getLabels().put(APPLICATION_LABEL_NAME, applicationName);
              if (!applicationName.equals(oldApplicationName)) {
            ...
            for (Route route : application.getRoutes()) {
              oldApplicationName =
                  route.getMetadata().getLabels().put(APPLICATION_LABEL_NAME, applicationName);
              if (!applicationName.equals(oldApplicationName)) {
