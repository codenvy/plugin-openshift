 ...
 */
@Singleton
public class ConfigureServicePresenter extends AbstractWizardPage<NewServiceRequest>
    implements ConfigureServiceView.ActionDelegate {

  private final ConfigureServiceView view;
  private final OpenshiftLocalizationConstant locale;

  @Inject
  public ConfigureServicePresenter(
      ConfigureServiceView view, OpenshiftLocalizationConstant locale) {
    this.view = view;
    this.locale = locale;
  ...
  private boolean isValidLabels() {
    for (KeyValue label : view.getEnvironmentLabels()) {
      if (!OpenshiftValidator.isLabelValueValid(label.getKey())
          || !OpenshiftValidator.isLabelValueValid(label.getValue())) {
