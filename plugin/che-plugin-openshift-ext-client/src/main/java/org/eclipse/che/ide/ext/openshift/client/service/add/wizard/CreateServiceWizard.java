...
public class CreateServiceWizard extends AbstractWizard<NewServiceRequest> {
  ...
  public CreateServiceWizard(
      @Assisted NewServiceRequest newServiceRequest,
  ...
  public void complete(final @NotNull CompleteCallback callback) {
    ...
    client
        .processTemplate(nameSpace, template)
          ...
          switch (kind) {
            ...
            case "DeploymentConfig":
              final DeploymentConfig deploymentConfig =
                  dtoFactory.createDtoFromJson(json.toString(), DeploymentConfig.class);
              deploymentConfig.getMetadata().setNamespace(nameSpace);
              Promise<JsArrayMixed> p =
                  client
                      .createDeploymentConfig(deploymentConfig)
  ...
  private Function<DeploymentConfig, Promise<JsArrayMixed>>
      linkApplicationDeloymentConfigsWithCreatedDeploymentConfig() {
    return new Function<DeploymentConfig, Promise<JsArrayMixed>>() {
      @Override
      public Promise<JsArrayMixed> apply(final DeploymentConfig serviceConfig)
          throws FunctionException {
        String applicationName = cdProject.getName();
        return client
            .getDeploymentConfigs(nameSpace, applicationName)
            .then(
                new Function<List<DeploymentConfig>, List<DeploymentConfig>>() {
                  @Override
                  public List<DeploymentConfig> apply(List<DeploymentConfig> configs)
                      throws FunctionException {
                    final String deploymentConfigName = serviceConfig.getMetadata().getName();
                    final List<Container> containersForUpdate =
                        serviceConfig.getSpec().getTemplate().getSpec().getContainers();
                    for (DeploymentConfig config : configs) {
                      List<Container> containers =
                          config.getSpec().getTemplate().getSpec().getContainers();
  ...
  private Function<List<DeploymentConfig>, Promise<JsArrayMixed>>
      updateApplicationDeploymentConfigs() {
