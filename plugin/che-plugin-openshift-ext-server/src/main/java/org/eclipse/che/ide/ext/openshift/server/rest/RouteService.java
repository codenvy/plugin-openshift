...
public class RouteService {
  ...
  public Route createRoute(@PathParam("namespace") String namespace, Route route)
    ...
    if (!ResourceKind.ROUTE.equals(route.getKind())) {
      throw new BadRequestException(
          route.getKind() + " cannot be handled as a " + ResourceKind.ROUTE);
    }
  ...
  public List<Route> getRoutes(
    ...
    List<IRoute> routes =
        clientFactory.getOpenshiftClient().list(ResourceKind.ROUTE, namespace, labels);
    return routes
        .stream()
  ...
  @Path("/{route}")
  @Produces(MediaType.APPLICATION_JSON)
  public Route getRoute(@PathParam("namespace") String namespace, @PathParam("route") String route)
      throws UnauthorizedException, ServerException {

    return toDto(
        Route.class, clientFactory.getOpenshiftClient().get(ResourceKind.ROUTE, route, namespace));
  }
  ...
  public Route updateRoute(
      @PathParam("namespace") String namespace, @PathParam("route") String routeName, Route route)
